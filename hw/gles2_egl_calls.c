...
extern int gles2_getMaxVertexAttrib(void);
int surf_id = 1;
/**
...
GLES2_CB(eglCreatePbufferSurface)
{
    ...
    if (!(fsurf = malloc(sizeof(*fsurf)))) {
        GLES2_PRINT("\tPbuffer creation failed!\n");
        GLES2_BARRIER_ARG;
        GLES2_BARRIER_RET;
        gles2_ret_TEGLSurface(s, 0);
        return;
    ...
    if((fsurf->surf = eglCreatePbufferSurface(dpy, config, attrib_list))
        == EGL_NO_SURFACE)
    {
        GLES2_PRINT("\tPbuffer creation failed!\n");
        free(fsurf);
...
GLES2_CB(eglCreateContext)
{
    ...
    if (hctx == EGL_NO_CONTEXT) {
        ...
        ctx->arrays = NULL;
        ctx->narrays = 0;
        ctx->ebo_list = malloc(sizeof(gles2_ebo));
        ctx->ebo_list->name = -1;
    ...
    if (ctx && !ctx->arrays) {
        if (ctx->client_type == EGL_OPENGL_ES_API) {
            if(ctx->client_version == 1) {
                ...
                ctx->narrays = gles2_getMaxVertexAttrib();
                ctx->arrays = malloc(ctx->narrays * sizeof(*ctx->arrays));
