...
#include "gles2_es20.def"

#undef CALL_ENTRY
#undef CALL_DUMMY

#define CALL_ENTRY(func) { #func, FNAME(func, es20,) },
#define CALL_DUMMY(func) { #func, FNAME(func, es20,) },

gles2_Call gles2_es20_calls[] =
{
#include "gles2_es20.def"
};
...
void gles2_es20_write(void *opaque, target_phys_addr_t addr, uint64_t value,
        unsigned size);
void gles2_es20_write(void *opaque, target_phys_addr_t addr, uint64_t value,
        unsigned size)
{
    ...
    GLES2_PRINT("Page %d write (call nr. %d) in ES20 Block\n", page, callnr);

    if (page > 1) {
        gles2_Client* client;
        gles2_Call const *call;
        call = gles2_es20_calls + callnr;
