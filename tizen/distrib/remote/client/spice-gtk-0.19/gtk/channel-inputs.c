...
#ifdef CONFIG_MARU
void spice_inputs_tizen_keydown(SpiceInputsChannel *channel, guint scancode, int spice_msgc_inputs_type)
{
    SpiceMsgcKeyDown down;
    SpiceMsgOut *msg;
    ...
    down.code = spice_make_scancode(scancode, FALSE);
    msg = spice_msg_out_new(SPICE_CHANNEL(channel), spice_msgc_inputs_type);
    msg->marshallers->msgc_inputs_tizen_keydown(msg->marshaller, &down);
    spice_msg_out_send(msg); 
}

void spice_inputs_tizen_keyup(SpiceInputsChannel *channel, guint scancode, int spice_msgc_inputs_type)
{
    SpiceMsgcKeyUp up;
    SpiceMsgOut *msg;
    ...
    up.code = spice_make_scancode(scancode, FALSE);
    msg = spice_msg_out_new(SPICE_CHANNEL(channel), spice_msgc_inputs_type);
    msg->marshallers->msgc_inputs_tizen_keyup(msg->marshaller, &up);
    spice_msg_out_send(msg);
}
#endif
...
/*
void spice_inputs_channel_hwkey(SpiceChannel *channel, int type, int keycode)
{
    SpiceMsgOut *msg;
    guint8 *buf;

    msg = spice_msg_out_new(channel, SPICE_MSGC_INPUTS_HWKEY_SCANCODE);
    buf = (guint8*)spice_marshaller_reserve_space(msg->marshaller, 2);
    buf[0] = type;
    buf[1] = keycode;
    spice_msg_out_send(msg);
