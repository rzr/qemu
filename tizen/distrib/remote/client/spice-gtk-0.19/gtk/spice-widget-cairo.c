...
#ifdef CONFIG_MARU
void rotate_display(cairo_t *cr, int type, int w, int h) {
    int degree = type * 90;

    cairo_translate(cr, w/2, h/2);
    cairo_rotate(cr, -(double)degree * ((double)M_PI/(double)180.0));

    if (type == ROTATION_180) {
       cairo_translate(cr, -w/2, -h/2);
    }
    else {
        cairo_translate(cr, -h/2, -w/2);
    }
}
#endif
...
void spicex_draw_event(SpiceDisplay *display, cairo_t *cr)
{
    ...
    if (d->ximage) {
        ...
        cairo_rectangle(cr, 0, 0, w, h); // image read start point
#ifdef CONFIG_MARU
        switch(d->rotation_type) {
        case 0:
            break;
        case 1:
            rotate_display(cr, ROTATION_90, w, h);
            break;
        case 2:
            rotate_display(cr, ROTATION_180, w, h);
            break;
        case 3:
            rotate_display(cr, ROTATION_270, w, h);
            break;
        default:
            break;
        }
