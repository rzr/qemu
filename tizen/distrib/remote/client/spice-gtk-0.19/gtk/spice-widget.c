...
#ifdef CONFIG_MARU
static void get_sdb_port(SpiceDisplay *display, int scancode, SendKeyType type, gboolean press_delayed)
{
    SpiceDisplayPrivate *d = SPICE_DISPLAY_GET_PRIVATE(display);
    ...
    switch (type) {
    case SEND_KEY_PRESS:
        spice_inputs_tizen_keydown(d->inputs, scancode, SPICE_MSGC_INPUTS_SDB_PORT);
        break;
    default:
        g_warn_if_reached();
    }
...
static void send_hwkey(SpiceDisplay *display, int scancode, SendKeyType type, gboolean press_delayed)
{
    SpiceDisplayPrivate *d = SPICE_DISPLAY_GET_PRIVATE(display);
    ...
    switch (type) {
    case SEND_KEY_PRESS:
        spice_inputs_tizen_keydown(d->inputs, scancode, SPICE_MSGC_INPUTS_HWKEY_DOWN);
        break;

    case SEND_KEY_RELEASE:
        spice_inputs_tizen_keyup(d->inputs, scancode, SPICE_MSGC_INPUTS_HWKEY_UP);
        break;
...
static void send_rotation(SpiceDisplay *display, int scancode, SendKeyType type, gboolean press_delayed)
{
    SpiceDisplayPrivate *d = SPICE_DISPLAY_GET_PRIVATE(display);
    ...
    switch (type) {
    case SEND_KEY_PRESS:
        d->rotation_type = scancode;
        spice_inputs_tizen_keydown(d->inputs, scancode, SPICE_MSGC_INPUTS_ROTATION_TYPE);
        break;
    default:
        g_warn_if_reached();
    }
...
static void send_hostkbd(SpiceDisplay *display, int scancode, SendKeyType type, gboolean press_delayed)
{
    SpiceDisplayPrivate *d = SPICE_DISPLAY_GET_PRIVATE(display);
    ...
    switch (type) {
    case SEND_KEY_PRESS:
        spice_inputs_tizen_keydown(d->inputs, scancode, SPICE_MSGC_INPUTS_HOSTKBD_ONOFF);
        break;
    default:
        g_warn_if_reached();
    }
...
static void send_close(SpiceDisplay *display, int scancode, SendKeyType type, gboolean press_delayed)
{
    SpiceDisplayPrivate *d = SPICE_DISPLAY_GET_PRIVATE(display);
    ...
    switch (type) {
    case SEND_KEY_PRESS:
        spice_inputs_tizen_keydown(d->inputs, scancode, SPICE_MSGC_INPUTS_CLOSE_TYPE);
        break;
    default:
        g_warn_if_reached();
    }
...
#ifdef CONFIG_MARU
void spice_display_get_sdbport(SpiceDisplay *display, const guint *keyvals,
                             int nkeyvals, SpiceDisplayKeyEvent kind)
{
    int i;
    ...
    if (kind & SPICE_DISPLAY_KEY_EVENT_CLICK) {
        for (i = 0 ; i < nkeyvals ; i++)
            get_sdb_port(display, keyvals[i], SEND_KEY_PRESS, FALSE);
    }
}

void spice_display_send_hwkeys(SpiceDisplay *display, const guint *keyvals,
                             int nkeyvals, SpiceDisplayKeyEvent kind)
{
    int i;
    ...
    if (kind & SPICE_DISPLAY_KEY_EVENT_PRESS) {
        for (i = 0 ; i < nkeyvals ; i++)
            send_hwkey(display, keyvals[i], SEND_KEY_PRESS, FALSE);
    }

    if (kind & SPICE_DISPLAY_KEY_EVENT_RELEASE) {
        for (i = (nkeyvals-1) ; i >= 0 ; i--)
            send_hwkey(display, keyvals[i], SEND_KEY_RELEASE, FALSE);
    }
}

void spice_display_send_rotation(SpiceDisplay *display, const guint *keyvals,
                             int nkeyvals, SpiceDisplayKeyEvent kind)
{
    int i;
    ...
    if (kind & SPICE_DISPLAY_KEY_EVENT_CLICK) {
        for (i = 0 ; i < nkeyvals ; i++)
            send_rotation(display, keyvals[i], SEND_KEY_PRESS, FALSE);
    }
}

void spice_display_send_hostkbd(SpiceDisplay *display, const guint *keyvals,
                             int nkeyvals, SpiceDisplayKeyEvent kind)
{
    int i;
    ...
    if (kind & SPICE_DISPLAY_KEY_EVENT_CLICK) {
        for (i = 0 ; i < nkeyvals ; i++)
            send_hostkbd(display, keyvals[i], SEND_KEY_PRESS, FALSE);
    }
}

void spice_display_send_close(SpiceDisplay *display, const guint *keyvals,
                             int nkeyvals, SpiceDisplayKeyEvent kind)
{
    int i;
    ...
    if (kind & SPICE_DISPLAY_KEY_EVENT_CLICK) {
        for (i = 0 ; i < nkeyvals ; i++)
            send_close(display, keyvals[i], SEND_KEY_PRESS, FALSE);
    }
}
#endif
                             ...
                             int *input_x, int *input_y)
{
    ...
    */
#ifdef CONFIG_MARU
    if (d->rotation_type == ROTATION_0 || d->rotation_type == ROTATION_180) {
        is = (double)(d->area.width-1) / (double)(display_w-1);
    }
    else {
        is = (double)(d->area.height-1) / (double)(display_w-1);
    }
#endif
    window_x -= display_x;
    window_y -= display_y;
...
static gboolean motion_event(GtkWidget *widget, GdkEventMotion *motion)
{
    ...
    case SPICE_MOUSE_MODE_CLIENT:
        ...
        else if (d->rotation_type == ROTATION_90) {
            if (x >= 0 && x < d->area.height &&
                y >= 0 && y < d->area.width) {
                    spice_inputs_position(d->inputs, d->area.width - y, x, get_display_id(display),
                                  button_mask_gdk_to_spice(motion->state));
            }
        }
        else if (d->rotation_type == ROTATION_180) {
            if (x >= 0 && x < d->area.width &&
                y >= 0 && y < d->area.height) {
                    spice_inputs_position(d->inputs, d->area.width - x, d->area.height - y, get_display_id(display),
                                  button_mask_gdk_to_spice(motion->state));
            } 
        }
        else if (d->rotation_type == ROTATION_270) {
            if (x >= 0 && x < d->area.height &&
                y >= 0 && y < d->area.width) {
                    spice_inputs_position(d->inputs, y, d->area.height - x, get_display_id(display),
                                  button_mask_gdk_to_spice(motion->state));
            }
        }
#else
...
#ifdef CONFIG_MARU
    if (d->rotation_type == ROTATION_90 || d->rotation_type == ROTATION_270) {
        if ((x < 0 || x >= d->area.height ||
             y < 0 || y >= d->area.width) &&
...
#ifdef CONFIG_MARU
        if (d->rotation_type == ROTATION_90 || d->rotation_type == ROTATION_270) {
            s = MIN ((double)wh / (double)fbw, (double)ww / (double)fbh);
        }
        else {
...
GdkPixbuf *spice_display_get_pixbuf(SpiceDisplay *display)
{
    ...
    pixbuf = gdk_pixbuf_new_from_data(data, GDK_COLORSPACE_RGB, false,
                                      ...
                                      (GdkPixbufDestroyNotify)g_free, NULL);
#ifdef CONFIG_MARU
    if (d->rotation_type == ROTATION_90) {
