...
#ifdef CONFIG_MARU
void virt_viewer_display_set_landscape(VirtViewerDisplay *display)
{
    VirtViewerDisplayPrivate *priv = display->priv;
    guint tmp = priv->desktopWidth;
    
    priv->desktopWidth = priv->desktopHeight; 
    priv->desktopHeight = tmp;
    priv->dirty = TRUE;
...
#ifdef CONFIG_MARU
void virt_viewer_display_send_hwkey_press(VirtViewerDisplay *display,
                                   const guint *keyvals, int nkeyvals)
{   
    g_return_if_fail(VIRT_VIEWER_IS_DISPLAY(display));
        
    VIRT_VIEWER_DISPLAY_GET_CLASS(display)->send_hwkey_press(display, keyvals, nkeyvals);
} 

void virt_viewer_display_send_hwkey_release(VirtViewerDisplay *display,
                                   const guint *keyvals, int nkeyvals)
{
    g_return_if_fail(VIRT_VIEWER_IS_DISPLAY(display));

    VIRT_VIEWER_DISPLAY_GET_CLASS(display)->send_hwkey_release(display, keyvals, nkeyvals);
}

void virt_viewer_display_send_rotation_type(VirtViewerDisplay *display,
                                   const guint *keyvals, int nkeyvals)
{
    g_return_if_fail(VIRT_VIEWER_IS_DISPLAY(display));

    VIRT_VIEWER_DISPLAY_GET_CLASS(display)->send_rotation_type(display, keyvals, nkeyvals);
}

void virt_viewer_display_send_hostkbd_onoff(VirtViewerDisplay *display,
                                   const guint *keyvals, int nkeyvals)
{
    g_return_if_fail(VIRT_VIEWER_IS_DISPLAY(display));

    VIRT_VIEWER_DISPLAY_GET_CLASS(display)->send_hostkbd_onoff(display, keyvals, nkeyvals);
}

void virt_viewer_display_send_close_type(VirtViewerDisplay *display,
                                   const guint *keyvals, int nkeyvals)
{
    g_return_if_fail(VIRT_VIEWER_IS_DISPLAY(display));

    VIRT_VIEWER_DISPLAY_GET_CLASS(display)->send_close_type(display, keyvals, nkeyvals);
}

void virt_viewer_display_req_sdb_port(VirtViewerDisplay *display,
                                   const guint *keyvals, int nkeyvals)
{           
    g_return_if_fail(VIRT_VIEWER_IS_DISPLAY(display));

    VIRT_VIEWER_DISPLAY_GET_CLASS(display)->req_sdb_port(display, keyvals, nkeyvals);                                  
}           
#endif
