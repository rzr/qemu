...
#ifdef CONFIG_MARU
void virt_viewer_window_hwkey_menu_press(GtkWidget *src, VirtViewerWindow *self);
void virt_viewer_window_hwkey_menu_release(GtkWidget *src, VirtViewerWindow *self);
void virt_viewer_window_hwkey_home_press(GtkWidget *src, VirtViewerWindow *self);
void virt_viewer_window_hwkey_back_press(GtkWidget *src, VirtViewerWindow *self);
void virt_viewer_window_hwkey_back_release(GtkWidget *src, VirtViewerWindow *self);
void virt_viewer_window_hwkey_home_release(GtkWidget *src, VirtViewerWindow *self);
void virt_viewer_window_hwkey_power_press(GtkWidget *src, VirtViewerWindow *self);
void virt_viewer_window_hwkey_power_release(GtkWidget *src, VirtViewerWindow *self);
void virt_viewer_window_hwkey_vol_up_press(GtkWidget *src, VirtViewerWindow *self);
void virt_viewer_window_hwkey_vol_up_release(GtkWidget *src, VirtViewerWindow *self);
void virt_viewer_window_hwkey_vol_down_press(GtkWidget *src, VirtViewerWindow *self);
void virt_viewer_window_hwkey_vol_down_release(GtkWidget *src, VirtViewerWindow *self);
void virt_viewer_window_hwkeys_press(const char *name, VirtViewerWindow *self);
void virt_viewer_window_hwkeys_release(const char *name, VirtViewerWindow *self);
void virt_viewer_window_menu_always_on_top(GtkWidget *menu, VirtViewerWindow *self);
#endif
...
static const struct keyComboDef keyCombos[] = {
    ...
    { { 2 }, 1, "Rotation 180"},
    { { 3 }, 1, "Rotation 270"},
    { { 1 }, 1, "Host keyboard on"}, // 11
    { { 0 }, 1, "Host keyboard off"},
    { { 1 }, 1, "Force close"}, // 13
    { { 0 }, 1, "Close"},
    { { 0 }, 1, "Shell"},
#else
    ...
    for (i = 0 ; i < G_N_ELEMENTS(keyCombos) ; i++) {
        if (!strcmp(text, keyCombos[i].label)) {
            ...
            if (i < 1) {
                ...
                virt_viewer_display_send_rotation_type(VIRT_VIEWER_DISPLAY(priv->display),
                                          keyCombos[i].keys,
                                          keyCombos[i].nkeys);

                gtk_window_get_size(GTK_WINDOW(priv->window), &width, &height);

                if ( *(keyCombos[i].keys) == 1 || *(keyCombos[i].keys) == 3) {
                    if (width < height) {
                        gtk_window_resize(GTK_WINDOW(priv->window), height, width);
                    } else {
                        gtk_window_reshow_with_initial_size(GTK_WINDOW(priv->window));
                ...
                virt_viewer_display_send_hostkbd_onoff(VIRT_VIEWER_DISPLAY(priv->display),
                                          keyCombos[i].keys,
                                          keyCombos[i].nkeys);
            } else if ( i >= 13 && i < 15) {
                virt_viewer_display_send_close_type(VIRT_VIEWER_DISPLAY(priv->display),
                                          keyCombos[i].keys,
                                          keyCombos[i].nkeys);
            } else if (i == 15) {
                virt_viewer_display_req_sdb_port(VIRT_VIEWER_DISPLAY(priv->display),
                                          keyCombos[i].keys,
                                          keyCombos[i].nkeys);
            }
#else
...
#ifdef CONFIG_MARU
G_MODULE_EXPORT void
virt_viewer_window_hwkey_menu_press(GtkWidget *src G_GNUC_UNUSED,
                                  VirtViewerWindow *self)
{
    virt_viewer_window_hwkeys_press("Menu", self);
}

G_MODULE_EXPORT void
virt_viewer_window_hwkey_home_press(GtkWidget *src G_GNUC_UNUSED,
                                  VirtViewerWindow *self)
{
    virt_viewer_window_hwkeys_press("Home", self);
}

G_MODULE_EXPORT void
virt_viewer_window_hwkey_back_press(GtkWidget *src G_GNUC_UNUSED,
                                  VirtViewerWindow *self)
{
    virt_viewer_window_hwkeys_press("Back", self);
}

G_MODULE_EXPORT void
virt_viewer_window_hwkey_power_press(GtkWidget *src G_GNUC_UNUSED,
                                  VirtViewerWindow *self)
{
    virt_viewer_window_hwkeys_press("Power", self);
}

G_MODULE_EXPORT void
virt_viewer_window_hwkey_vol_up_press(GtkWidget *src G_GNUC_UNUSED,
                                  VirtViewerWindow *self)
{
    virt_viewer_window_hwkeys_press("Volume+", self);
}

G_MODULE_EXPORT void
virt_viewer_window_hwkey_vol_down_press(GtkWidget *src G_GNUC_UNUSED,
                                  VirtViewerWindow *self)
{
    virt_viewer_window_hwkeys_press("Volume-", self);
}
...
G_MODULE_EXPORT void
virt_viewer_window_hwkeys_press(const char *name, VirtViewerWindow *self)
{
    int i = 0;
    VirtViewerWindowPrivate *priv = self->priv;
    for (i = 0 ; i < G_N_ELEMENTS(keyCombos) ; i++) {
        if (!strcmp(name, keyCombos[i].label)) {
            virt_viewer_display_send_hwkey_press(VIRT_VIEWER_DISPLAY(priv->display),
                                          keyCombos[i].keys,
                                          keyCombos[i].nkeys);
        }
    }
...
G_MODULE_EXPORT void
virt_viewer_window_hwkey_menu_release(GtkWidget *src G_GNUC_UNUSED,
                                  VirtViewerWindow *self)
{
    virt_viewer_window_hwkeys_release("Menu", self);
}

G_MODULE_EXPORT void
virt_viewer_window_hwkey_home_release(GtkWidget *src G_GNUC_UNUSED,
                                  VirtViewerWindow *self)
{
    virt_viewer_window_hwkeys_release("Home", self);
}

G_MODULE_EXPORT void
virt_viewer_window_hwkey_back_release(GtkWidget *src G_GNUC_UNUSED,
                                  VirtViewerWindow *self)
{
    virt_viewer_window_hwkeys_release("Back", self);
}

G_MODULE_EXPORT void
virt_viewer_window_hwkey_power_release(GtkWidget *src G_GNUC_UNUSED,
                                  VirtViewerWindow *self)
{
    virt_viewer_window_hwkeys_release("Power", self);
}

G_MODULE_EXPORT void
virt_viewer_window_hwkey_vol_up_release(GtkWidget *src G_GNUC_UNUSED,
                                  VirtViewerWindow *self)
{
    virt_viewer_window_hwkeys_release("Volume+", self);
}

G_MODULE_EXPORT void
virt_viewer_window_hwkey_vol_down_release(GtkWidget *src G_GNUC_UNUSED,
                                  VirtViewerWindow *self)
{
    virt_viewer_window_hwkeys_release("Volume-", self);
}
...
G_MODULE_EXPORT void
virt_viewer_window_hwkeys_release(const char *name, VirtViewerWindow *self)
{
    int i = 0;
    VirtViewerWindowPrivate *priv = self->priv;
    for (i = 0 ; i < G_N_ELEMENTS(keyCombos) ; i++) {
        if (!strcmp(name, keyCombos[i].label)) {
            virt_viewer_display_send_hwkey_release(VIRT_VIEWER_DISPLAY(priv->display),
                                          keyCombos[i].keys,
                                          keyCombos[i].nkeys);
        }
    }
...
#ifdef CONFIG_MARU
G_MODULE_EXPORT void
virt_viewer_window_menu_always_on_top(GtkWidget *menu,
                                    VirtViewerWindow *self)
{
    VirtViewerWindowPrivate *priv = self->priv;
...
#ifdef CONFIG_MARU
    gtk_widget_set_no_show_all(priv->toolbar, FALSE);
#else
...
#ifdef CONFIG_MARU
    /* HwKey Volume- */
    button = GTK_WIDGET(gtk_tool_button_new_from_stock(GTK_STOCK_GO_DOWN));
    gtk_tool_item_set_tooltip_text(GTK_TOOL_ITEM(button), _("Volume-"));
    gtk_toolbar_insert(GTK_TOOLBAR(priv->toolbar), GTK_TOOL_ITEM (button), 0);
    g_signal_connect(button, "clicked", G_CALLBACK(virt_viewer_window_hwkey_vol_down_press), self);
    g_signal_connect(button, "clicked", G_CALLBACK(virt_viewer_window_hwkey_vol_down_release), self);
    gtk_widget_show(GTK_WIDGET(button));

     /* HwKey Volume+ */
    button = GTK_WIDGET(gtk_tool_button_new_from_stock(GTK_STOCK_GO_UP));
    gtk_tool_item_set_tooltip_text(GTK_TOOL_ITEM(button), _("Volume+"));
    gtk_toolbar_insert(GTK_TOOLBAR(priv->toolbar), GTK_TOOL_ITEM (button), 0);
    g_signal_connect(button, "clicked", G_CALLBACK(virt_viewer_window_hwkey_vol_up_press), self);
    g_signal_connect(button, "clicked", G_CALLBACK(virt_viewer_window_hwkey_vol_up_release), self);
    gtk_widget_show(GTK_WIDGET(button));

     /* HwKey Power */
    button = GTK_WIDGET(gtk_tool_button_new_from_stock(GTK_STOCK_CONNECT));
    gtk_tool_item_set_tooltip_text(GTK_TOOL_ITEM(button), _("Power"));
    gtk_toolbar_insert(GTK_TOOLBAR(priv->toolbar), GTK_TOOL_ITEM (button), 0);
    g_signal_connect(button, "clicked", G_CALLBACK(virt_viewer_window_hwkey_power_press), self);
    g_signal_connect(button, "clicked", G_CALLBACK(virt_viewer_window_hwkey_power_release), self);
    gtk_widget_show(GTK_WIDGET(button));

    /* HwKey Back */
    button = GTK_WIDGET(gtk_tool_button_new_from_stock(GTK_STOCK_GO_BACK));
    gtk_tool_item_set_tooltip_text(GTK_TOOL_ITEM(button), _("Back"));
    gtk_toolbar_insert(GTK_TOOLBAR(priv->toolbar), GTK_TOOL_ITEM (button), 0);
    gtk_widget_set_events(button, GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK);
    g_signal_connect(button, "clicked", G_CALLBACK(virt_viewer_window_hwkey_back_press), self);
    g_signal_connect(button, "clicked", G_CALLBACK(virt_viewer_window_hwkey_back_release), self);
    gtk_widget_show(GTK_WIDGET(button));

    /* HwKey Home */
    button = GTK_WIDGET(gtk_tool_button_new_from_stock(GTK_STOCK_HOME));
    gtk_tool_item_set_tooltip_text(GTK_TOOL_ITEM(button), _("Home"));
    gtk_toolbar_insert(GTK_TOOLBAR(priv->toolbar), GTK_TOOL_ITEM (button), 0);
    gtk_widget_set_events(button, GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK);
    g_signal_connect(button, "clicked", G_CALLBACK(virt_viewer_window_hwkey_home_press), self);
    g_signal_connect(button, "clicked", G_CALLBACK(virt_viewer_window_hwkey_home_release), self);
    gtk_widget_show(GTK_WIDGET(button));

    /* HwKey Menu */
    button = GTK_WIDGET(gtk_tool_button_new_from_stock(GTK_STOCK_INDEX));
    gtk_tool_item_set_tooltip_text(GTK_TOOL_ITEM(button), _("Menu"));
    gtk_toolbar_insert(GTK_TOOLBAR(priv->toolbar), GTK_TOOL_ITEM (button), 0);
    gtk_widget_set_events(button, GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK);
    g_signal_connect(button, "clicked", G_CALLBACK(virt_viewer_window_hwkey_menu_press), self);
    g_signal_connect(button, "clicked", G_CALLBACK(virt_viewer_window_hwkey_menu_release), self);
    gtk_widget_show(GTK_WIDGET(button));
#endif
