...
#ifdef CONFIG_MARU
#define SDK_PATH_FILE "/.installmanager/multisdkpath"
#define TOOLS_SDB "/tools/sdb"

static uint8_t* open_SdbShell(uint16_t port)
{
    FILE* fd;
    char SDK_path[256];
    char multisdkpath[128];
    char* home = getenv("HOME");
    char sdb_path[512];
    int res;
    char sdb_port[10];

    sprintf(sdb_port, "%d", port);
    strcpy(multisdkpath, home);

    /* to find sdb binary path */ 
    strcat(multisdkpath, SDK_PATH_FILE);
    fd = fopen(multisdkpath, "r");
    if(fd < 0) {
        return (uint8_t*)1;
    }

    res = fscanf(fd, "%s", SDK_path);
    fclose(fd);
    if(res != 1) {
        return (uint8_t*)res;
    }
    
    strcpy(sdb_path, SDK_path);
    strcat(sdb_path, TOOLS_SDB);

    char connect_cmd[1024];
    strcpy(connect_cmd, sdb_path);
    strcat(connect_cmd, " connect ");
    strcat(connect_cmd, HOST_IP);
    strcat(connect_cmd, ":");
    strcat(connect_cmd, sdb_port);
    res = system(connect_cmd);
    if(res != 0) {
        return (uint8_t*)res;
    } 

    char excute_cmd[1024] = "gnome-terminal -e \"";
    strcat(excute_cmd, sdb_path);
    strcat(excute_cmd, " -e -s ");
    strcat(excute_cmd, HOST_IP);
    strcat(excute_cmd, ":");
    strcat(excute_cmd, sdb_port);
    strcat(excute_cmd, " shell\"");
    res = system(excute_cmd);
    if(res != 0) {
        return (uint8_t*)res;
    }
...
#ifdef CONFIG_MARU
    if (message_type >= 26100) {
        return open_SdbShell(message_type + 1);
    }
#endif
    if (message_type >= 1 && message_type < 9) {
        return funcs1[message_type-1](message_start, message_end, minor, size_out, free_message);
