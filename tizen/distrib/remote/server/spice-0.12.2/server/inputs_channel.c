...
#ifdef CONFIG_MARU
struct SpiceSdbPortState {
    int dummy;
};

struct SpiceHwkeyState {
    int dummy;
};

struct SpiceRotationState {
    int dummy;
};

struct SpiceHostkbdState {
    int dummy;
};

struct SpiceTizenCloseState {
    int dummy;
};
#endif
...
#ifdef CONFIG_MARU
static SpiceSdbPortInstance *sdbport = NULL;
static SpiceHwkeyInstance *hwkey = NULL;
static SpiceRotationInstance *rotation = NULL;
static SpiceHostkbdInstance *hostkbd = NULL;
static SpiceTizenCloseInstance *close = NULL;
#endif
...
#ifdef CONFIG_MARU
int inputs_get_sdbport(SpiceSdbPortInstance *_sdbport)
{   
    if (sdbport) {
        spice_printerr("already have sdb port");
        return -1;
    }       
    sdbport = _sdbport;
    sdbport->st = spice_new0(SpiceSdbPortState, 1);
    return 0;
} 
...
{
    if (hwkey) {
        spice_printerr("already have hwkey");
        return -1;
    }
    hwkey = _hwkey;
    hwkey->st = spice_new0(SpiceHwkeyState, 1);
    return 0;
}

int inputs_set_rotation(SpiceRotationInstance *_rotation)
{
    if (rotation) {
        spice_printerr("already have rotation");
        return -1;
    }
    rotation = _rotation;
    rotation->st = spice_new0(SpiceRotationState, 1);
    return 0;
}

int inputs_set_hostkbd(SpiceHostkbdInstance *_hostkbd)
{
    if (hostkbd) {
        spice_printerr("already have hostkbd");
        return -1;
    }
    hostkbd = _hostkbd;
    hostkbd->st = spice_new0(SpiceHostkbdState, 1);
    return 0;
}

int inputs_set_tizen_close(SpiceTizenCloseInstance *_close)
{
    if (close) {
        spice_printerr("already have tizen close");
        return -1;
    }
    close = _close;
    close->st = spice_new0(SpiceTizenCloseState, 1);
    return 0;
}
#endif
...
#ifdef CONFIG_MARU
static int sdbport_get_scan(SpiceSdbPortInstance *sin)
{
    SpiceSdbPortInterface *sif;
    int sdb_port = -1;
    if (!sin) {
        return sdb_port;
    }
    sif = SPICE_CONTAINEROF(sin->base.sif, SpiceSdbPortInterface, base);
    sif->push_scan_freg(sin, &sdb_port);    
    spice_printerr("sdb port: %d", sdb_port);

    return sdb_port;
}

static void hwkey_push_scan(SpiceHwkeyInstance *sin, uint8_t type, uint8_t keycode)
{
    SpiceHwkeyInterface *sif;
    if (!sin) {
        return;
    }
    sif = SPICE_CONTAINEROF(sin->base.sif, SpiceHwkeyInterface, base);
    sif->push_scan_freg(sin, type, keycode);
    spice_printerr("type: %d, keycode: %d", type, keycode);
}

static void rotation_push_scan(SpiceRotationInstance *sin, uint8_t type)
{
    SpiceRotationInterface *sif;
    if (!sin) {
        return;
    }
    sif = SPICE_CONTAINEROF(sin->base.sif, SpiceRotationInterface, base);
    sif->push_scan_freg(sin, type);
    spice_printerr("type: %d", type);
}

static void hostkbd_push_scan(SpiceHostkbdInstance *sin, uint8_t type)
{
    SpiceHostkbdInterface *sif;
    if (!sin) {
        return;
    }
    sif = SPICE_CONTAINEROF(sin->base.sif, SpiceHostkbdInterface, base);
    sif->push_scan_freg(sin, type);
    spice_printerr("type: %d", type);
}

static void close_push_scan(SpiceTizenCloseInstance *sin, uint8_t type)
{
    SpiceTizenCloseInterface *sif;
    if (!sin) {
        return;
    }
    sif = SPICE_CONTAINEROF(sin->base.sif, SpiceTizenCloseInterface, base);
    sif->push_scan_freg(sin, type);
    spice_printerr("type: %d", type);
}
#endif
                                        ...
                                        void *message)
{
    ...
    switch (type) {
#ifdef CONFIG_MARU
    case SPICE_MSGC_INPUTS_SDB_PORT: {
        int sdb_port = -1;
        sdb_port = sdbport_get_scan(sdbport);
        /* send sdb port to client */
        rcc->sdb_port = sdb_port;
        rcc->outgoing.cb = &rcc->channel->outgoing_cb;
        rcc->send_data.header.data = malloc(sizeof(uint8_t));
        rcc->send_data.header.set_msg_type(&rcc->send_data.header, PIPE_ITEM_TYPE_SDB_PORT);
        red_channel_client_push(rcc);
        break;
    }
    case SPICE_MSGC_INPUTS_HWKEY_DOWN: {
        uint8_t *code = (uint8_t *)buf;
        hwkey_push_scan(hwkey, 1, code[0]);
        break;
    }
    case SPICE_MSGC_INPUTS_HWKEY_UP: {
        uint8_t *code = (uint8_t *)buf;
        hwkey_push_scan(hwkey, 2, code[0]);
        break;
    }
    case SPICE_MSGC_INPUTS_ROTATION_TYPE: {
        uint8_t *code = (uint8_t *)buf;
        rotation_push_scan(rotation, code[0]);
        break;
    }
    case SPICE_MSGC_INPUTS_HOSTKBD_ONOFF: {
        uint8_t *code = (uint8_t *)buf;
        hostkbd_push_scan(hostkbd, code[0]);
        break;
    }
    case SPICE_MSGC_INPUTS_CLOSE_TYPE: {
        uint8_t *code = (uint8_t *)buf;
        close_push_scan(close, code[0]);
        break;
    }
#endif
