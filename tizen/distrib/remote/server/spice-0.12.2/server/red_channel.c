...
#ifdef CONFIG_MARU
typedef struct SdbPortPipeItem {
    PipeItem base;
    int msg;
} SdbPortPipeItem;
#endif
...
//            return;
        handler->size = handler->vec->iov_len;
        }
    }
...
#ifdef CONFIG_MARU
static void red_channel_client_send_sdb_port(RedChannelClient *rcc, PipeItem *base)
{
    SdbPortPipeItem *msg_pipe_item = SPICE_CONTAINEROF(base, SdbPortPipeItem, base);
...
static void red_channel_client_send_item(RedChannelClient *rcc, PipeItem *item)
{
    ...
    switch (item->type) {
        ...
        case PIPE_ITEM_TYPE_EMPTY_MSG:
            ...
            break;
#ifdef CONFIG_MARU
        case PIPE_ITEM_TYPE_SDB_PORT:
            red_channel_client_send_sdb_port(rcc, item);
            break;
#endif
        default:
            handled = FALSE;
...
#ifdef CONFIG_MARU
    if (rcc->sdb_port) {
        item = (PipeItem *)&rcc->pipe.prev;
        item->type = PIPE_ITEM_TYPE_SDB_PORT; 
        return item;
    }
    else if (!rcc || rcc->send_data.blocked
...
#ifdef CONFIG_MARU
    if (!rcc->during_send || rcc->sdb_port) {
