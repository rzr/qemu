...
#ifdef CONFIG_MARU
    if (strcmp(interface->type, SPICE_INTERFACE_HWKEY) == 0) {
        spice_info("SPICE_INTERFACE_HWKEY");
        if (interface->major_version != SPICE_INTERFACE_HWKEY_MAJOR ||
            interface->minor_version > SPICE_INTERFACE_HWKEY_MINOR) {
            spice_warning("unsupported hwkey interface");
            return -1;
        }
        if (inputs_set_hwkey(SPICE_CONTAINEROF(sin, SpiceHwkeyInstance, base)) != 0) {
            return -1;
        }
    } else if (strcmp(interface->type, SPICE_INTERFACE_SDBPORT) == 0) {
        spice_info("SPICE_INTERFACE_SDBPORT");
        if (interface->major_version != SPICE_INTERFACE_SDBPORT_MAJOR ||
            interface->minor_version > SPICE_INTERFACE_SDBPORT_MINOR) {
            spice_warning("unsupported sdb port interface");
            return -1;
        }
        if (inputs_get_sdbport(SPICE_CONTAINEROF(sin, SpiceSdbPortInstance, base)) != 0) {
            return -1;
        }
    } else if (strcmp(interface->type, SPICE_INTERFACE_ROTATION) == 0) {
        spice_info("SPICE_INTERFACE_ROTATION");
        if (interface->major_version != SPICE_INTERFACE_ROTATION_MAJOR ||
            interface->minor_version > SPICE_INTERFACE_ROTATION_MINOR) {
            spice_warning("unsupported rotation interface");
            return -1;
        }
        if (inputs_set_rotation(SPICE_CONTAINEROF(sin, SpiceRotationInstance, base)) != 0) {
            return -1;
        }
    } else if (strcmp(interface->type, SPICE_INTERFACE_HOSTKBD) == 0) {
        spice_info("SPICE_INTERFACE_HOSTKBD");
        if (interface->major_version != SPICE_INTERFACE_HOSTKBD_MAJOR ||
            interface->minor_version > SPICE_INTERFACE_HOSTKBD_MINOR) {
            spice_warning("unsupported hostkbd interface");
            return -1;
        }
        if (inputs_set_hostkbd(SPICE_CONTAINEROF(sin, SpiceHostkbdInstance, base)) != 0) {
            return -1;
        }
    } else if (strcmp(interface->type, SPICE_INTERFACE_TIZEN_CLOSE) == 0) {
        spice_info("SPICE_INTERFACE_TIZEN_CLOSE");
        if (interface->major_version != SPICE_INTERFACE_TIZEN_CLOSE_MAJOR ||
            interface->minor_version > SPICE_INTERFACE_TIZEN_CLOSE_MINOR) {
            spice_warning("unsupported tizen close interface");
            return -1;
        }
        if (inputs_set_tizen_close(SPICE_CONTAINEROF(sin, SpiceTizenCloseInstance, base)) != 0) {
            return -1;
        }
    }
#endif
