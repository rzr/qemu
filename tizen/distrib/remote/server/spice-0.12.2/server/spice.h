...
/* for TIZEN-maru */
#define SPICE_INTERFACE_SDBPORT "sdbport"
#define SPICE_INTERFACE_SDBPORT_MAJOR 1
#define SPICE_INTERFACE_SDBPORT_MINOR 1

typedef struct SpiceSdbPortInterface SpiceSdbPortInterface;
typedef struct SpiceSdbPortInstance SpiceSdbPortInstance;
typedef struct SpiceSdbPortState SpiceSdbPortState; 

struct SpiceSdbPortInterface {
    SpiceBaseInterface base;
        
    void (*push_scan_freg)(SpiceSdbPortInstance *sin, int* sdb_port);
    uint8_t (*get_leds)(SpiceSdbPortInstance *sin);
};  
    
struct SpiceSdbPortInstance {
    SpiceBaseInstance base;
    SpiceSdbPortState     *st;
};

#define SPICE_INTERFACE_HWKEY "hwkey"
#define SPICE_INTERFACE_HWKEY_MAJOR 1
#define SPICE_INTERFACE_HWKEY_MINOR 1

typedef struct SpiceHwkeyInterface SpiceHwkeyInterface;
typedef struct SpiceHwkeyInstance SpiceHwkeyInstance;
typedef struct SpiceHwkeyState SpiceHwkeyState;

struct SpiceHwkeyInterface {
    SpiceBaseInterface base;

    void (*push_scan_freg)(SpiceHwkeyInstance *sin, uint8_t type, uint8_t keycode);
    uint8_t (*get_leds)(SpiceHwkeyInstance *sin);
};

struct SpiceHwkeyInstance {
    SpiceBaseInstance base;
    SpiceHwkeyState     *st;
};

#define SPICE_INTERFACE_ROTATION "rotation"
#define SPICE_INTERFACE_ROTATION_MAJOR 1
#define SPICE_INTERFACE_ROTATION_MINOR 1

typedef struct SpiceRotationInterface SpiceRotationInterface;
typedef struct SpiceRotationInstance SpiceRotationInstance;
typedef struct SpiceRotationState SpiceRotationState;
    
struct SpiceRotationInterface {
    SpiceBaseInterface base;

    void (*push_scan_freg)(SpiceRotationInstance *sin, uint8_t type);
    uint8_t (*get_leds)(SpiceRotationInstance *sin);
};  
    
struct SpiceRotationInstance {
    SpiceBaseInstance base;
    SpiceRotationState     *st;
};

#define SPICE_INTERFACE_HOSTKBD "hostkbd"
#define SPICE_INTERFACE_HOSTKBD_MAJOR 1
#define SPICE_INTERFACE_HOSTKBD_MINOR 1

typedef struct SpiceHostkbdInterface SpiceHostkbdInterface;
typedef struct SpiceHostkbdInstance SpiceHostkbdInstance;
typedef struct SpiceHostkbdState SpiceHostkbdState;

struct SpiceHostkbdInterface {
    SpiceBaseInterface base;

    void (*push_scan_freg)(SpiceHostkbdInstance *sin, uint8_t type);
    uint8_t (*get_leds)(SpiceHostkbdInstance *sin);
};

struct SpiceHostkbdInstance {
    SpiceBaseInstance base;
    SpiceHostkbdState     *st;
};

#define SPICE_INTERFACE_TIZEN_CLOSE "close"
#define SPICE_INTERFACE_TIZEN_CLOSE_MAJOR 1
#define SPICE_INTERFACE_TIZEN_CLOSE_MINOR 1

typedef struct SpiceTizenCloseInterface SpiceTizenCloseInterface;
typedef struct SpiceTizenCloseInstance SpiceTizenCloseInstance;
typedef struct SpiceTizenCloseState SpiceTizenCloseState;

struct SpiceTizenCloseInterface {
    SpiceBaseInterface base;

    void (*push_scan_freg)(SpiceTizenCloseInstance *sin, uint8_t type);
    uint8_t (*get_leds)(SpiceTizenCloseInstance *sin);
};

struct SpiceTizenCloseInstance {
    SpiceBaseInstance base;
    SpiceTizenCloseState     *st;
};
/* for TIZEN-maru */
