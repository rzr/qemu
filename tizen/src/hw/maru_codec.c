...
int qemu_avcodec_open (SVCodecState *s)
{
    ...
    memcpy(avctx, s->vram_ptr, size);
    memcpy(&tmpCodec, (uint8_t*)s->vram_ptr + size, sizeof(AVCodec));
    ...
    if (avctx->extradata_size > 0) {
        avctx->extradata = (uint8_t*)av_malloc(avctx->extradata_size);
        memcpy(avctx->extradata, (uint8_t*)s->vram_ptr + size, avctx->extradata_size);
    } else {
        avctx->extradata = NULL;
    }
    ...
    memcpy(s->vram_ptr, avctx, sizeof(AVCodecContext));
    memcpy((uint8_t*)s->vram_ptr + sizeof(AVCodecContext), &ret, sizeof(int));
...
int qemu_avcodec_close (SVCodecState* s)
{
    ...
    if (!avctx) {
        ...
        memcpy(s->vram_ptr, &ret, sizeof(int));
        return ret;
    }
...
int qemu_avcodec_decode_video (SVCodecState *s)
{
    ...
    memcpy(&buf_size, (uint8_t*)s->vram_ptr, sizeof(int));
    size = sizeof(int);
    ...
    size = sizeof(AVCodecContext);
    memcpy(s->vram_ptr, avctx, size);
    memcpy((uint8_t*)s->vram_ptr + size, picture, sizeof(AVFrame));
    size += sizeof(AVFrame);
    memcpy((uint8_t*)s->vram_ptr + size, &got_picture_ptr, sizeof(int));
    size += sizeof(int);
    memcpy((uint8_t*)s->vram_ptr + size, &ret, sizeof(int));
...
int qemu_avcodec_encode_video (SVCodecState *s)
{
    ...
    size = sizeof(int);
    memcpy(&bPict, s->vram_ptr, size);
    TRACE("avframe is :%d\n", bPict);

    if (bPict == 0) {
        memcpy(&outputBufSize, (uint8_t*)s->vram_ptr + size, size);
        size += sizeof(int);

        memcpy(pict, (uint8_t*)s->vram_ptr + size, sizeof(AVFrame));
        size += sizeof(AVFrame);

        numBytes = avpicture_get_size(avctx->pix_fmt, avctx->width, avctx->height);
        TRACE("input buffer size :%d\n", numBytes);

        inputBuf = (uint8_t*)s->vram_ptr + size;
        if (!inputBuf) {
    ...
    ret = avcodec_encode_video (avctx, s->vram_ptr, outputBufSize, pict);

    TRACE("after encoding video, ret:%d\n");
    if (ret < 0) {
        ERR("Failed to encode video\n");
    }
    memcpy((uint8_t*)s->vram_ptr + outputBufSize, &ret, sizeof(int));
...
void qemu_av_picture_copy (SVCodecState *s)
{
    ...
    memcpy(s->vram_ptr, dst.data[0], numBytes);
...
int qemu_av_parser_parse (SVCodecState *s)
{
    ...
    size = sizeof(AVCodecContext);
    memcpy(avctx, s->vram_ptr, sizeof(AVCodecContext));
    qemu_restore_context(avctx, &tmp_ctx);
    memcpy(&pts, (uint8_t*)s->vram_ptr + size, sizeof(int64_t));
    size += sizeof(int64_t);
    memcpy(&dts, (uint8_t*)s->vram_ptr + size, sizeof(int64_t));
    size += sizeof(int64_t);
    memcpy(&inbuf_size, (uint8_t*)s->vram_ptr + size, sizeof(int));
    if (inbuf_size > 0) {
        size += sizeof(int);
        inbuf = (uint8_t*)s->vram_ptr + size;
        size += inbuf_size;
    } else {
        inbuf = NULL;
    }
    ...
    memcpy((uint8_t*)s->vram_ptr + size, &poutbuf_size, sizeof(int));
    size += sizeof(int);
    if (poutbuf_size != 0) {
        memcpy((uint8_t*)s->vram_ptr + size, poutbuf, poutbuf_size);
        size += poutbuf_size;
    }
    memcpy((uint8_t*)s->vram_ptr + size, &ret, sizeof(int));
...
static void codec_write (void *opaque, target_phys_addr_t addr, uint32_t value, unsigned size)
{
    uint32_t offset;
    int ret = -1;
...
static int codec_initfn (PCIDevice *dev)
{
    ...
    memory_region_init_ram(&s->vram, NULL, "codec.ram", SVCODEC_MEM_SIZE);
    s->vram_ptr = memory_region_get_ram_ptr(&s->vram);

    /* FIX ME */
    memory_region_init_io (&s->mmio, &codec_mmio_ops, s, "codec-mmio", SVCODEC_REG_SIZE);
...
static int codec_exitfn (PCIBus *bus)
{
    SVCodecState *s = DO_UPCAST(SVCodecState, dev, dev);
